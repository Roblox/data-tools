...
import org.apache.spark.sql.types.{AbstractDataType, BinaryType, DataType, NumericType, StringType}
...
case class ThetaSketchBuildAggregate(child: Expression,
  ...
  def this(child: Expression) =
    this(child, ThetaSketchConfig(), 0, 0)
  ...
  override def serialize(buffer: ThetaSketch): Array[Byte] = buffer.toByteArray

  override def deserialize(bytes: Array[Byte]): ThetaSketch = ThetaSketch(bytes, config.nominalEntities)
